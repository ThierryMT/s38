...
def upload_folder_to_r2(r2, bucket, prefix="", max_workers=8):
    ...
    ensure_bucket_exists(r2, bucket, logger=None)
...
def ensure_bucket_exists(r2_client: BaseClient, bucket_name: str, logger=None):
    """
    Ensure an R2 bucket exists, creating it if necessary.
    
    Args:
        r2_client: The R2 client (with write permissions)
        bucket_name: Name of the bucket to check/create
        logger: Optional logger for messages
    
    Returns:
        True if bucket exists or was created, False if creation failed
    """
    try:
        ...
        return True
    except r2_client.exceptions.NoSuchBucket:
        # Bucket doesn't exist, try to create it
        try:
            if logger:
                logger.info(f"🔨 Creating new R2 bucket: {bucket_name}")
        ...
        # If we can't check, assume it exists and let upload fail with proper error
        return True
...
def log_peerid_to_r2(self, prefix=""):
    if self.master:
        ...
        if not ensure_bucket_exists(self.r2["write"], bucket_name, self.logger):
            self.logger.error(f"❌ Cannot upload peer ID: bucket {bucket_name} does not exist and could not be created")
            self.logger.error(f"Please manually create the bucket in Cloudflare R2 dashboard or check R2 write permissions")
            raise RuntimeError(f"Bucket {bucket_name} does not exist and auto-creation failed")
